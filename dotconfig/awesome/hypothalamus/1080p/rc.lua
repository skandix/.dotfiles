...
mytextclock = awful.widget.textclock(markup("#00daff", "%A %d %B ") .. markup("#343639", ">") .. markup("#de5e1e", " %H:%M:%S "))
...
yawn = lain.widgets.yawn(858537, {
    settings = function()
        widget:set_markup(markup("#eb00ff", forecast:lower() .. " @ " .. units .. "°C "))
    end
})
...
cpuwidget = lain.widgets.cpu({
    settings = function()
        widget:set_markup(markup("#d10088", cpu_now.usage .. "% "))
    end
})
...
tempwidget = lain.widgets.temp({
    settings = function()
        widget:set_markup(markup("#f38600", coretemp_now .. "°C "))
    end
})
...
volumewidget = lain.widgets.alsa({
    settings = function()
        ...
        widget:set_markup(markup("#002BFF", volume_now.level .. "% "))
    end
})
...
netupinfo = lain.widgets.net({
    settings = function()
        ...
        widget:set_markup(markup("#FF0000", net_now.sent .. " "))
        netdowninfo:set_markup(markup("#00FF1A", net_now.received .. " "))
    end
})
...
memwidget = lain.widgets.mem({
    settings = function()
        widget:set_markup(markup("#FFFC00", mem_now.used .. "M "))
    end
})
...
client.connect_signal("manage", function (c, startup)
    ...
    if titlebars_enabled and (c.type == "normal" or c.type == "dialog") then
        ...
        awful.titlebar(c,{size=13}):set_widget(layout)
    end
end)
